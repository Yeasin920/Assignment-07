export default function Footer() {
  return (
    <footer className="bg-gray-800 text-white text-center p-4 mt-4 w-[1600px]">
        <div className="flex justify-around mb-10">
             <div>
                <h1 className="text-[24px] font-bold">CS — Ticket System</h1>
                <p className="text-gray-400">Lorem Ipsum is simply dummy text of the <br /> printing and typesetting industry. Lorem <br /> Ipsum has been the industry's standard <br /> dummy text ever since the 1500s, when an <br /> unknown printer took a galley of type and <br /> scrambled it to make a type specimen book.</p>
             </div>
             
              <div>
                <h2 className="text-xl">Company</h2>
                <ul className="text-gray-400">
                    <li><a href=""></a>About Us</li>
                     <li><a href="">Our Mission</a></li>
                      <li><a href="">Contact Saled</a></li>
                      
                </ul>
              </div>
                   <div>
                <h2 className="text-xl">Services</h2>
                <ul className="text-gray-400">
                    <li><a href=""></a>Products & Services</li>
                     <li><a href="">Customer Stories</a></li>
                      <li><a href="">Download Apps</a></li> 
                </ul>
              </div>

                 <div>
                <h2 className="text-xl">Information</h2>
                <ul className="text-gray-400">
                    <li><a href=""></a>Privacy Policy</li>
                     <li><a href="">Terms & Conditions</a></li>
                      <li><a href="">Join Us</a></li> 
                </ul>
              </div>
                
                   <div>
                <h2 className="text-xl">Social Links</h2>
                <ul className="text-gray-400">
                    <li><a href=""></a><i class="fa-brands fa-x-twitter"></i> @CS — Ticket System</li>
                     <li><a href=""><i class="fa-brands fa-linkedin"></i> @CS — Ticket System</a></li>
                      <li><a href=""><i class="fa-brands fa-facebook"></i> @CS — Ticket System</a></li> 
                    <li><a href=""><i class="fa-regular fa-envelope"></i> support@cst.com</a></li> 
                </ul>
              </div>

               
        </div>
      © 2025 CS — Ticket System. All rights reserved.
    </footer>
  );
}
